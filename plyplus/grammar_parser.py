...
def p_tokendef(p):
    ...
    """
    if len(p) > 5:
        p[0] = T('tokendef', (p[1], p[3], p[4]))
    else:
        p[0] = T('tokendef', (p[1], p[3]))
...
def p_optiondef(p):
    ...
    if len(p) == 6:
        assert p[1] == '%fragment'
        p[0] = T('fragmentdef', (p[2], p[4]))
    else:
        p[0] = T('optiondef', (p[1], p[3]))

def p_rules_list(p):
    """rules_list   : production
                    | production OR rules_list"""
    p[0] = T('rules_list', [p[1]] + p[3:])
...
def p_perm_rule(p):
    ...
    if len(p) == 2:
        ...
        p[0] = T('perm_rule', (p[1], p[3]))

def p_perm_phrase(p):
    """perm_phrase : rule PERM rule
                   | rule PERM perm_phrase
    """
    p[0] = T('perm_phrase', [p[1]] + p[3:])
